...
from orchidarium.bus import InterfaceClaim, read
...
def main() -> int:
    ...
    with InterfaceClaim(device, detach=True):
        ...
        _extract_temperature: re.Pattern = re.compile(r'^(?<=T:\s)[0-9]+.[0-9]+(?=,)')
        _extract_relative_humidity: re.Pattern = re.compile(r'(?<=RH: )[0-9]+.[0-9]+(?=\r\n)$')


        if re.match(_match, (_res := read(device[0][(0,0)][0], device).decode('utf-8', errors='ignore'))):
            print(_res)
            temperature = re.search(_extract_temperature, _res).group()
            print(temperature)
            humidity = re.search(_extract_relative_humidity, _res).group()
            print(humidity)
            return 0
        return 1
